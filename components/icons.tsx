...
export function Icon({ name, size = 18, stroke = 'currentColor', strokeWidth = 2, fill = 'none', className = '' }: IconProps) {
  const props = { width: size, height: size, viewBox: '0 0 24 24', fill, stroke, strokeWidth, strokeLinecap: 'round' as const, strokeLinejoin: 'round' as const, className }
  switch (name) {
    ...
    case 'star':         return <svg {...props} fill="currentColor"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
    ...
    case 'heart-fill':   return <svg {...props} fill="currentColor"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
    ...
    case 'factory':      return <svg {...props}><path d="M2 20h20"/><path d="M14 12V8l-4 3V8l-4 3V4H4v16h16V12z"/><line x1="7" y1="16" x2="7" y2="16"/><line x1="11" y1="16" x2="11" y2="16"/><line x1="15" y1="16" x2="15" y2="16"/></svg>
    ...
    case 'play':         return <svg {...props} fill="currentColor"><polygon points="5 3 19 12 5 21 5 3"/></svg>
    ...
    case 'verified':     return <svg {...props} viewBox="0 0 24 24"><path d="M12 2l2.39 1.85 2.94-.49 1.39 2.64 2.64 1.39-.49 2.94L22 12l-1.85 2.39.49 2.94-2.64 1.39-1.39 2.64-2.94-.49L12 22l-2.39-1.85-2.94.49-1.39-2.64-2.64-1.39.49-2.94L2 12l1.85-2.39-.49-2.94 2.64-1.39 1.39-2.64 2.94.49L12 2z" fill={fill !== 'none' ? fill : 'currentColor'} stroke="none"/><polyline points="9 12 11 14 15 10" stroke="white" strokeWidth={2.2}/></svg>
...
export function CategoryIcon({ name, size = 22 }: { name: string; size?: number }) {
  return <Icon name={CATEGORY_ICON_MAP[name] || 'box'} size={size} />
}
